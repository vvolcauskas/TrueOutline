// trueOutline.jsx v0.97

workDoc = app.documents[0];
exportDoc = app.documents["Export.ai"];

function logError(msg) {
    var logFile = new File("Desktop/illustrator_log.txt");
    logFile.open("a");
...
for (layer=0; layer < workDoc.layers.length; layer++) {
    if (workDoc.layers[layer].visible == true) { //current layer is visible
        ...
        var availableFontTypes = ["Font", "Font2", "FontInv", "FontLine", "FontDrop"];
        var fontType;
...
var layerOptions = {
    "Inv": { // NOT SWITCHING BETWEEN INV AND NORMAL TEXT????
        ...
        "minus": ["Line", "Bck", "Drop", "Color0", "Color1", "Color2", "Color0Const", "Color1Const", "Color2Const"] // PathFinder MinusFront for art in WorkingDesign
    },
    "Drop": {
        "cookieCutter": false,
        "addOffsetText": false,
    ...
    "Color0": {
        ...
        "minusText": true,
        "add": [],
        "minus": ["Line", "Drop", "Bck", "Color1", "Color2", "MetalBack", "MetalBack2", "MetalBack3", "MetalBack4", "MetalBack5", "MetalBack6"]
    },
    "Color1": {
        "cookieCutter": false,
        "addOffsetText": false,
        ...
        "minusText": true,
        "add": [],
        "minus": ["Line", "Drop", "Bck", "Color0", "Color2", "MetalBack", "MetalBack2", "MetalBack3", "MetalBack4", "MetalBack5", "MetalBack6"]
    },
    "Color2": {
        "cookieCutter": false,
        "addOffsetText": false,
        ...
        "minusText": true,
        "add": [],
        "minus": ["Line", "Drop", "Bck", "Color0", "Color1", "MetalBack", "MetalBack2", "MetalBack3", "MetalBack4", "MetalBack5", "MetalBack6"]
    },
    "Color0Const": {
        "cookieCutter": false,
        "addOffsetText": false,
        ...
        "minusText": true,
        "add": [],
        "minus": ["Line", "Drop", "Bck", "MetalBack", "MetalBack2", "MetalBack3", "MetalBack4", "MetalBack5", "MetalBack6"]
    },
    "Color1Const": {
        "cookieCutter": false,
        "addOffsetText": false,
...
function workLayer(layer, options) {
    ...
    for (i=0; i<count; i++) {
        try {
            ...
            } else if (options["minus"][i] == "Bck" || options["minus"][i] == "BckConst") {
                deselect();
                exportDoc.layers["Working"].groupItems["Bck"].selected = true;
                app.doScript("fixOffset", "trueOutline"); //fixes bad overlap from pathfinder
            }
        } catch (e) {
    ...
    selectAll("all");
    if (toMinus == true) {
        // alert("Minusing " + layer.name + " from " + options["minus"]); // debug
...
// fontDrop type, copy and translate font
if (fontType == "FontDrop") {
    deselect();

    // translate working font
    exportDoc.layers["Working"].compoundPathItems[0].translate(2, -2); //translate x,y

    // rename to Drop and group
    deselect();
    exportDoc.layers["Working"].compoundPathItems[0].selected = true;
    app.executeMenuCommand("group");
    exportDoc.layers["Working"].groupItems[0].name = "Drop";

    // move to workingDesign
    exportDoc.layers["Working"].groupItems[0].move(exportDoc.layers["WorkingDesign"], ElementPlacement.PLACEATBEGINNING);

    // add to final layers lists
    artNames.push("Drop");
...
if (fontType == "FontLine") {
    ...
    redraw();
    app.doScript("minusFront", "trueOutline"); //minus template from text
    app.doScript("makeCompoundPath", "trueOutline"); //convert to compoundPath
...
// group all groups with same name
var groupNames = {};
i = 0;
while (i < exportDoc.layers.length) {
    var layer = exportDoc.layers[i];
    var layerName = layer.name;

    if (layerName in groupNames) {
        // Layer name already exists, group the items
        exportDoc.layers[i].groupItems[0].move(exportDoc.layers[groupNames[layerName]], ElementPlacement.PLACEATBEGINNING);
        exportDoc.layers[i].remove();
    } else {
        groupNames[layerName] = i;
        i++;
    }
}