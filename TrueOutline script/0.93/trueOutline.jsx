    ...
    if (workDoc.layers[layer].visible == true) { //current layer is visible
        ...
        for (i=0; i<count; i++) {
            if (workDoc.layers[layer].layers["Design"].groupItems[i].hidden == false) {
                ...
                if (workDoc.layers[layer].layers["Design"].groupItems[i].name != "Template") { // item not used in final design
...
if (count > 0) {
    ...
    if (fontType == "FontLine") {
        ...
        // stop();
        // // should create 1 group with many paths
        // // loop through vertices of paths to find highest y, remove the rest of the paths

        // for (item = 0; item < exportDoc.layers["Working"].pageItems.length; item++) {
        //     alert(exportDoc.layers["Working"].pageItems[item].typename)
        // }
    }
